...
//        assertEquals("2017-04-17", json.getString("bookingdates.checkout"));
        ...
        //3.way Soft assertion (Soft assert class comes from testng)
        //to use soft assertion follow given 3 steps
        //1) create softAssert object
        SoftAssert softAssert = new SoftAssert();

        //2)By using  softAssert object do assertion
        softAssert.assertEquals(json.getString("firstname"),"Mark","Firstname didn't match");
        softAssert.assertTrue(json.getString("lastname").equals("Jackson"),"Lastname did not match");
        softAssert.assertEquals(json.getInt("totalprice"),538,"Total price did not match");
        softAssert.assertFalse(json.getBoolean("depositpaid"),"Deposit paid is not false");
        softAssert.assertEquals(json.getString("bookingdates.checkin"),"2021-03-22","Check-in date did not match");
        softAssert.assertEquals(json.getString("bookingdates.checkout"),"2022-01-31","Check out date did not match");
